...
var M2Server = function(overrideOptions) {
    ...
    var deleteClient = function(clientID) {
        runShellCommand('perl-scripts/remove_user.pl ' + clients[clientID].systemUserName +
            ' ' + clients[clientID].schrootName + ' ' + clients[clientID].schrootType, function(
            ret) {
    ...
    var restartAction = function(request, response) {
        assureClient(request, response, function(clientID) {
            ...
            if (client.m2) {
                ...
                runShellCommand("killall -u " + clients[clientID].systemUserName, function(
                    ret) {
    ...
    var checkForEventStream = function(clientID, response) {
        if (!clients[clientID].eventStream || clients[clientID].eventStream.length ==
            0) {
    ...
    var saveAction = function(request, response) {
        assureClient(request, response, function(clientID) {
            ...
            request.on("end", function() {
                ...
                var msg = {
                    input: path + "Macaulay2-input",
                    output: path + "Macaulay2-output"
                };
    ...
    var app = connect()
        ...
        .use('/usr/local/var/lib/schroot/mount', connect.static(
