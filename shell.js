...
var process = function(userName, schroot, ondata){
   ...
   var init;
   if(schroot){
      this.init = function(command, ondata){
        var spawn = require('child_process').spawn;
            ...
            */
            var pipe = spawn('sudo', ['cgexec', '-g', 'cpu,memory:' + clients[clientID].systemUserName,
                    'sudo', '-u', clients[clientID].systemUserName, 'nice', 'schroot', '-c', clients[clientID].schrootName,
                                    '-u', clients[clientID].systemUserName, '-d', '/home/m2user/', '-r', command
            ]);
      
        pipe.on('exit', function(returnCode, signal) {
            // the schroot might still be valid or unmounted
            logClient(clientID, command+" exited.");
            logClient(clientID, "returnCode: " + returnCode);
            logClient(clientID, "signal: " + signal);
            pipe.stdout.removeAllListeners('data');
            pipe.stderr.removeAllListeners('data');
        });
        ...
        if(pipe){
            pipe.stdout.removeAllListeners('data');
            pipe.stderr.removeAllListeners('data');
            pipe.stdout.on('data', ondata);
            pipe.stderr.on('data', ondata);
